...
def verify_min_shift_criteria(doc_info, start_date, end_date, scheduling_month,scheduling_year):
    ...
    if end_date == last_day_of_month(scheduling_year, scheduling_month):
        ...
        if(doc_info["period_no_of_shifts"]<min_criteria_period):
            return False
    return True

def generate_working_options(doctor,doc_info_history,doc_info,roster,scheduling_month,scheduling_year,start_date, end_date):
        ...
        for shift in doc_info_history[day]:
            if day <= end_date and doctor not in roster[day][shift]:
                modified_doc_info={
                    ...
                    "period_no_of_night_shifts":doc_info["period_no_of_night_shifts"],
                    "period_no_of_day_shifts":doc_info["period_no_of_day_shifts"],
...
def second_stage_update_doc_info(doctor,docs_info,docs_info_history,day,shift,scheduling_month, scheduling_year, start_date, end_date):
    doc_info=docs_info[doctor]

    next_shift_day,next_shift_shift=get_next_shift(day,shift,end_date)
    if(next_shift_day!=-1):
        ...
        temp_day,temp_shift=get_next_shift(next_shift_day,next_shift_shift,end_date)
...
def check_eligible_future(doc_info,doc_info_history ,day, shift, start_date, end_date):
    next_to_next_shift_day,next_to_next_shift_shift=get_next_shift(day,shift,end_date,step=2)
    ...
    five_days_ahead_day,five_days_ahead_shift=get_next_shift(day+1,"day",end_date,step=10)
    ...
    if shift=="night":
        if day+3<end_date and doc_info_history[day+3]["night"]==2:
            return False
    return True

def create_stage_two_roster(teams, off_requests,scheduling_month, start_date, end_date, scheduling_year,docs_info,docs_info_history,roster, scheduling_half=0):
        ...
        for doctor in doc_working_options:
            ...
            while True:
                ...
                if option_set:
                    ...
                    docs_info,docs_info_history=second_stage_update_doc_info(doctor,docs_info,docs_info_history,chosen_shift[0],chosen_shift[1],scheduling_month, scheduling_year, start_date, end_date)
                    break
